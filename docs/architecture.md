...
Loaded schema data, ordered processors, processor configuration, compiled constraints, and other reusable metadata belong to the immutable schema or pipeline rather than the per-event processing context. Ordered attribute and constraint names are initialized once beside their class or object definitions. Validation-enabled pipelines additionally initialize the parsed schema version; compiled type regular expressions; resolved range and maximum-length constraints; and allowed values normalized to their OCSF primitive types. Enrichment-only pipelines do not pay that cost. A processing context contains only mutable state for one event plus references to shared immutable data. Processing must not copy processor collections, rebuild reusable traversal metadata, or perform other avoidable per-event work merely to recover information already owned by the pipeline.
