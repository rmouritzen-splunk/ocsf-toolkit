...
- Replace eager validation and enrichment path strings with an event-local reusable path stack backed by a slice with initial capacity for eight attribute/array segments and normal doubling when deeper traversal requires it. Render indexed validation paths or index-free enrichment paths only when a diagnostic, observable, or other retained result needs a string; path views must not outlive their callback. Preserve arbitrary nesting, nested arrays, sibling paths, and deterministic diagnostics. Prefer this maintainable representation initially; consider inline storage or cross-event reuse only if later memory profiles justify the added complexity. Use the existing simple, nested-array, observable-heavy, and malformed-event benchmarks to verify allocation and runtime improvements.
