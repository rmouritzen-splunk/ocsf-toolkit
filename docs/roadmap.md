...
## Multiple schema versions before 1.0

Support processing a mixed-version event stream with multiple loaded compiled schemas before stabilizing the 1.0 public API. For example, one environment may contain events whose `metadata.version` values are `1.5.0` and `1.8.0`; selecting the corresponding compiled schema for each event should produce more accurate enrichment, enrichment removal, and validation than processing every event against one schema version.

Resolve the public API design before 1.0 because the current `Schema.NewPipeline` ownership model assumes one schema and could require a backward-incompatible change if multi-schema selection is added later. Preserve the existing single-schema use case where practical, but do not constrain the multi-schema design merely to avoid a pre-1.0 API correction.
