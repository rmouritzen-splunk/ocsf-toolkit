...
func (p *eventProcessorImpl) ProcessEvent(event jsonish.Map) (ProcessingResult, error) {
	return p.schema.processEvent(event, p.config)
}

func (si *schemaImpl) processEvent(event jsonish.Map, config processorConfig) (ProcessingResult, error) {
	if event == nil {
	...
	if context.validationEnabled() && context.class.Deprecated != nil {
		context.addWarning(
			"class_deprecated",
			fmt.Sprintf(
	...
	context.processItem("", "", event, &context.class.commonItemDefinition, false)
	...
	if context.addObservables() && len(context.observables) > 0 {
...
func (c *processingContext) validationEnabled() bool {
	return c.config.validation.enabled
}

func (c *processingContext) warnOnMissingRecommended() bool {
	return c.config.validation.warnOnMissingRecommended
}

func (c *processingContext) addEnumSiblings() bool {
	return c.config.enrichment.enabled && c.config.enrichment.addEnumSiblings
}

func (c *processingContext) addObservables() bool {
	return c.config.enrichment.enabled && c.config.enrichment.addObservables
}
...
func (c *processingContext) resolveClass(event jsonish.Map) {
	...
	if !present {
		if c.validationEnabled() {
			c.addRequiredAttributeMissing("class_uid", "class_uid")
		}
		c.stopped = true
		return
	}
	if !ok || classUID < math.MinInt32 || classUID > math.MaxInt32 {
		if c.validationEnabled() {
			c.addWrongType("class_uid", "class_uid", event["class_uid"], "integer_t", "")
		}
		c.stopped = true
		return
	}

	class, classPresent := c.classes[int32(classUID)]
	if !classPresent {
		if c.validationEnabled() {
			c.addError(
				"class_uid_unknown",
				fmt.Sprintf("Unknown \"class_uid\" value; no class is defined for %d.", classUID),
				jsonish.Map{
...
func (c *processingContext) validateAndReturnProfiles(event jsonish.Map) []string {
	...
	for index, profileValue := range profiles {
		...
		if c.validationEnabled() {
			if _, present := c.profiles[profile]; !present {
...
func (c *processingContext) processItem(
	...
	for _, attributeName := range attributeNames {
		...
		if !present || value == nil {
			if c.validationEnabled() {
				c.validateRequirement(validationPath, attributeName, attrDef)
			}
			continue
		}
	...
	if c.validationEnabled() {
		c.validateUnknownKeys(item, validationParentPath, itemDefinition, filteredAttributes)
		if validateItemConstraints {
			c.validateConstraints(item, itemDefinition, validationParentPath)
		}
	}
}
...
func (c *processingContext) processAttribute(
	...
	if c.validationEnabled() && attrDef.Deprecated != nil {
		c.addWarning(
			"attribute_deprecated",
			fmt.Sprintf("Attribute %q is deprecated. %s", attributeName, attrDef.Deprecated.Message),
			jsonish.Map{
...
func (c *processingContext) processValue(
	...
	if attrDef.Enum != nil {
		c.processEnum(containingItem, value, validationPath, attributeName, attrDef, arrayIndex)
	}
	...
	if c.validationEnabled() {
		c.validatePrimitiveValue(value, validationPath, attributeName, attrDef)
	}

	if attrDef.Enum == nil && c.addObservables() {
		if typeID, present := c.getObservableTypeID(enrichmentPath, attributeName, attrDef); present {
			c.addValueObservable(enrichmentPath, typeID, value)
		}
	}
}
...
func (c *processingContext) processObjectValue(
	...
	if !ok {
		if c.validationEnabled() {
			...
			c.addWrongType(validationPath, attributeName, value, expectedType, "")
		}
		return
	}
	...
	if !present || objectDef == nil {
		if c.validationEnabled() {
			c.addError(
				...
				fmt.Sprintf("SCHEMA BUG: Object %q is not defined.", *attrDef.ObjectType),
				jsonish.Map{
					"attribute_path": validationPath,
					"attribute":      attributeName,
					"object_type":    *attrDef.ObjectType,
					"value":          value,
				},
			)
		}
	...
	if c.addObservables() && objectDef.Observable != nil {
		c.addObjectObservable(enrichmentPath, *objectDef.Observable)
	}

	if c.validationEnabled() && objectDef.Deprecated != nil {
		c.addWarning(
			"object_deprecated",
			fmt.Sprintf("Object %q is deprecated. %s", objectDef.Name, objectDef.Deprecated.Message),
			jsonish.Map{
				"attribute_path": validationPath,
				"attribute":      attributeName,
	...
	c.processItem(validationPath, enrichmentPath, objectValue, &objectDef.commonItemDefinition, true)
}

func (c *processingContext) processEnum(
	containingItem jsonish.Map,
	value any,
	validationPath string,
	attributeName string,
	...
	valueString := coerce.StringLenient(value)
	enumDetail, enumPresent := attrDef.Enum[valueString]

	if c.addEnumSiblings() && arrayIndex < 0 {
		c.addEnumSibling(containingItem, valueString, enumDetail, attrDef)
	}
	...
	if arrayIndex >= 0 {
		c.validateEnumArraySibling(containingItem, value, validationPath, attributeName, attrDef, enumDetail, arrayIndex)
		c.validateEnumArrayValueDeprecated(value, validationPath, attributeName, enumDetail)
	} else {
		c.validateEnumSibling(containingItem, value, validationPath, attributeName, attrDef, enumDetail)
		c.validateEnumValueDeprecated(value, validationPath, attributeName, enumDetail)
	}
}
...
func (c *processingContext) validateNumberRange(
	...
	case int64:
		outside = value < int64(low) || value > int64(high)
	case float64:
	...
	if typeName != attributeTypeName {
		...
		details["super_type"] = typeName
		details["super_type_range"] = []int32{low, high}
		message = fmt.Sprintf(
...
func (c *processingContext) validateTypeUID(event jsonish.Map) {
	...
	expectedTypeUID := classUID*100 + activityID
...
func (c *processingContext) addObjectObservable(attributePath string, observableTypeID int32) {
...
func (c *processingContext) addValueObservable(attributePath string, observableTypeID int32, value any) {
